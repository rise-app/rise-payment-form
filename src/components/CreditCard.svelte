<script>
  ...
  $: cardMonth = cardMonth < minCardMonth
      ? ''
      : cardMonth
  $: minCardMonth = cardYear === minCardYear
      ? new Date().getMonth() + 1
      : 1
  ...
  $: {
    ...
    else if (cardNumber.match(new RegExp("^4")) != null) {
      cardType = "visa"
    }
    else {
      cardType = "rise"
    ...
    for (let index = 0; index < cardNumber.length; index++) {
      if (cardNumberMask[index] == ' ' && cardNumber[index] !== ' ') {
        cardNumber = cardNumber.substr(0, index) + ' ' + cardNumber.substr(index, cardNumber.length-index)
      }
    }
        ...
        <div class="card-item__side back">
          ...
          <div class="card-item__cvv">
            ...
            <div class="card-item__type">
              {#if cardType}
                {#each [cardType] as cardType (cardType)}
                  <img in:fly={{y:-20}} out:fly={{y:20}} alt="card" src={'images/' + cardType + '.png'} class="card-item__typeImg">
                {/each}
              {/if}
            </div>
          </div>
